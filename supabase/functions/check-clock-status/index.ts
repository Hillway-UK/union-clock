...
serve(async (req) => {
  ...
  try {
    ...
    console.log(
      `Running check-clock-status at ${new Date().toISOString()}, UK time: ${dateStr} ${timeHHmm}, day: ${dayOfWeek}`,
    );
  ...
  for (const worker of workers) {
    ...
    if (clockedIn) {
      ...
      await cancelNotifications(
        ...
        worker.id,
        siteDate,
        `clock_in_.*_shift${worker.shift_start.replace(":", "")}`,
      );
      continue;
    }
  ...
  for (const worker of workers) {
    ...
    if (!stillClockedIn) {
      ...
      await cancelNotifications(
        ...
        worker.id,
        siteDate,
        `clock_out_.*_shift${worker.shift_end.replace(":", "")}`,
      );
      continue;
    }
...
async function handleAutoClockOut(
  ...
  for (const worker of workers) {
    ...
    if (activeOT) {
      console.log(
        `Worker ${worker.name} has active overtime entry (${activeOT.id}), skipping time-based auto-clockout for main shift`,
      );
    ...
    const shiftEndFormatted = new Intl.DateTimeFormat("en-GB", {
      hour: "numeric",
      minute: "2-digit",
      hour12: true,
    }).format(new Date(`1970-01-01T${worker.shift_end}`));

    await sendNotification(
      supabase,
      worker.id,
      "⚠️ Auto Clocked-Out: Shift Ended",
      `You were automatically clocked out at ${clockOutFormatted} because you did not clock out by your scheduled shift end time (${shiftEndFormatted}).\n\n📅 Clock-Out Time: ${clockOutFormatted}\n\n\nIf you worked longer than your shift, please submit a Time Amendment from your timesheet.`,
...
async function createAudit(supabase: any, workerId: string, shiftDate: Date, performed: boolean, reason: string) {
  const { error } = await supabase.from("auto_clockout_audit").insert({
    worker_id: workerId,
...
async function logNotification(supabase: any, workerId: string, notifType: string, siteDate: Date) {
...
async function cancelNotifications(supabase: any, workerId: string, siteDate: Date, typePattern: string) {
  ...
  const idsToCancel = logs.filter((log: any) => regex.test(log.notification_type)).map((log: any) => log.id);

  if (idsToCancel.length > 0) {
    await supabase.from("notification_log").update({ canceled: true }).in("id", idsToCancel);
  }
}
...
async function sendNotification(
  ...
  try {
    ...
    const dedupeKey =
      type.includes("auto_clockout") || type.includes("fallback")
